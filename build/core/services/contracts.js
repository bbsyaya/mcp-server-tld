...
export function getReadableFunctions(abi) {
    return abi
        ...
        const outputs = (item.outputs || []).map((i) => `${i.type} ${i.name || ""}`).join(", ");
...
export async function multicall(params, network = "mainnet") {
    ...
    try {
        const callDataWithFuncs = calls.map((call) => {
            ...
            const encodedArgs = tronWeb.utils.abi
 ...
 */
export async function deployContract(privateKey, params, network = "mainnet") {
    const tronWeb = getWallet(privateKey, network);
    try {
        ...
        const transaction = await tronWeb.transactionBuilder.createSmartContract(deploymentOptions, tronWeb.defaultAddress.hex);
        ...
        if (result && result.result) {
            ...
            if (!contractAddress)
                throw new Error("Contract deployed but failed to resolve address");
...
export async function estimateEnergy(params, network = "mainnet") {
    ...
    try {
        ...
        if (!ownerAddress) {
            throw new Error("Missing ownerAddress for energy estimation. Provide an address or configure a wallet.");
        }
        ...
        if (matched.length > 1) {
            const overloads = matched
                .map((item) => `${params.functionName}(${(item.inputs || []).map((i) => i.type).join(", ")})`)
                .join(" | ");
            throw new Error(`Ambiguous overload for ${params.functionName} with ${args.length} argument(s). Candidates: ${overloads}`);
        }
