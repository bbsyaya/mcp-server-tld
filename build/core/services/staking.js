 ...
 */
export async function freezeBalanceV2(privateKey, amount, resource = "BANDWIDTH", network = "mainnet") {
    const tronWeb = getWallet(privateKey, network);
    try {
        const transaction = await tronWeb.transactionBuilder.freezeBalanceV2(amount, resource, tronWeb.defaultAddress.base58 || undefined);
 ...
 * @returns Transaction hash.
 */
export async function unfreezeBalanceV2(privateKey, amount, resource = "BANDWIDTH", network = "mainnet") {
    const tronWeb = getWallet(privateKey, network);
    try {
        const transaction = await tronWeb.transactionBuilder.unfreezeBalanceV2(amount, resource, tronWeb.defaultAddress.base58 || undefined);
        const signedTx = await tronWeb.trx.sign(transaction, privateKey);
        const result = await tronWeb.trx.sendRawTransaction(signedTx);
        if (result.result)
            return result.txid;
 ...
 * @returns Transaction hash.
 */
export async function withdrawExpireUnfreeze(privateKey, network = "mainnet") {
    const tronWeb = getWallet(privateKey, network);
    try {
        const transaction = await tronWeb.transactionBuilder.withdrawExpireUnfreeze(tronWeb.defaultAddress.base58 || undefined);
        const signedTx = await tronWeb.trx.sign(transaction, privateKey);
        const result = await tronWeb.trx.sendRawTransaction(signedTx);
        if (result.result)
            return result.txid;
