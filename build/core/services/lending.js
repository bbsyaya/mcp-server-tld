...
export async function borrow(privateKey, jTokenSymbol, amount, network = "mainnet") {
    ...
    const assetsIn = await comptroller.methods.getAssetsIn(walletAddress).call();
    if (assetsIn.length === 0) {
        throw new Error(`No borrowing capacity. You need to supply assets and enable them as collateral (enter_market) before borrowing.`);
    }
    ...
    if (availableLiquidityUSD <= 0) {
        const breakdown = collateralDetails.map(d => `${d.symbol}: supply=$${d.supplyValueUSD.toFixed(2)} × CF ${(d.collateralFactor * 100).toFixed(0)}% = $${d.adjustedValueUSD.toFixed(2)}` +
            (d.borrowBalanceUSD > 0 ? `, borrow=$${d.borrowBalanceUSD.toFixed(2)}` : "")).join("; ");
        throw new Error(`No borrowing capacity. Total adjusted collateral: $${totalAdjustedCollateralUSD.toFixed(2)}, ` +
    ...
    if (amountRaw > maxBorrowableRaw) {
        const breakdown = collateralDetails.map(d => `${d.symbol}: supply=$${d.supplyValueUSD.toFixed(2)} × CF ${(d.collateralFactor * 100).toFixed(0)}% = $${d.adjustedValueUSD.toFixed(2)}` +
            (d.borrowBalanceUSD > 0 ? `, borrow=$${d.borrowBalanceUSD.toFixed(2)}` : "")).join("; ");
        throw new Error(`Insufficient borrowing capacity. Requested: ${amount} ${info.underlyingSymbol} (~$${(parseFloat(amount) * targetPriceUSD).toFixed(2)}), ` +
 ...
 */
function encodeParams(tronWeb, params) {
 ...
 */
async function trySimulateEnergy(tronWeb, ownerAddress, contractAddress, functionSelector, params, options = {}, _network = "mainnet") {
    ...
    const [estimateResult, simResult] = await Promise.allSettled([
        tronWeb.transactionBuilder.estimateEnergy(contractAddress, functionSelector, options, params, ownerAddress),
 ...
 */
async function estimateApproveStep(tronWeb, ownerAddress, tokenAddress, spenderAddress, tokenSymbol, spenderLabel, requiredAmount, network) {
    const sim = (addr, fn, params, opts = {}) => trySimulateEnergy(tronWeb, ownerAddress, addr, fn, params, opts, network);
 ...
 */
export async function estimateLendingEnergy(operation, jTokenSymbol, amount, ownerAddress, network = "mainnet", spender) {
    const tronWeb = getTronWeb(network);
    const addresses = getJustLendAddresses(network);
    const steps = [];
    const sim = (addr, fn, params, opts = {}) => trySimulateEnergy(tronWeb, ownerAddress, addr, fn, params, opts, network);
    ...
    switch (operation) {
        case "approve": {
            ...
            const result = await estimateApproveStep(tronWeb, ownerAddress, info.underlying, spenderAddr, info.underlyingSymbol, spenderLabel, undefined, network);
            if (result)
        ...
        case "supply": {
            const amountRaw = BigInt(Math.floor(parseFloat(amount) * 10 ** info.underlyingDecimals));
            if (!isTRX && info.underlying) {
                const approveResult = await estimateApproveStep(tronWeb, ownerAddress, info.underlying, info.address, info.underlyingSymbol, jTokenSymbol, amountRaw, network);
                if (approveResult) {
 ...
 */
export async function simulateOperationResources(operation, jTokenSymbol, amount, ownerAddress, network = "mainnet") {
    const typical = getTypicalResources(operation, operation === "supply" || operation === "repay"
        ? (() => {
            try {
                const i = resolveJToken(jTokenSymbol, network);
                return i.underlyingSymbol === "TRX" || !i.underlying;
            }
            catch {
                return false;
            }
