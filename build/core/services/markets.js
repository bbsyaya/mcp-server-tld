 ...
 */
export async function getMarketDataFromAPI(network = "mainnet") {
 ...
 */
async function getJTokenDetailFromAPI(jtokenAddr, network = "mainnet") {
    const host = getApiHost(network);
 ...
 */
export async function getAllMarketOverview(network = "mainnet") {
    const host = getApiHost(network);
    // Fetch markets list
    const marketsResp = await fetch(`${host}/justlend/markets`);
    if (!marketsResp.ok)
        throw new Error(`Markets API failed: ${marketsResp.status}`);
    const marketsData = await marketsResp.json();
    if (marketsData.code !== 0)
        throw new Error(`Markets API error: ${marketsData.code}`);
    const jtokenList = marketsData.data?.jtokenList || [];
    if (jtokenList.length === 0)
        throw new Error("No markets returned from API");
    // Filter active markets (isValid=1)
    const activeMarkets = jtokenList.filter((m) => m.isValid === "1" || m.isValid === 1);
    // Fetch jToken details for each market (includes mining data)
    const detailPromises = activeMarkets.map((m) => getJTokenDetailFromAPI(m.jtokenAddress, network));
 ...
 */
export async function getMarketDashboardFromAPI(network = "mainnet") {
    const host = getApiHost(network);
 ...
 */
export async function getJTokenDetailsFromAPI(jtokenAddr, network = "mainnet") {
    const host = getApiHost(network);
    const url = `${host}/justlend/markets/jtokenDetails?jtokenAddr=${jtokenAddr}`;
    try {
        const response = await fetch(url);
