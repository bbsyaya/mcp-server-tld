import { z } from "zod";
import { getJustLendAddresses, getAllJTokens, getJTokenInfo, getSupportedNetworks, } from "./chains.js";
 ...
 */
export function registerJustLendTools(server) {
    // ============================================================================
    ...
    server.registerTool("get_market_data", {
        description: "Get detailed market data for a specific JustLend market: supply/borrow APY, TVL, utilization, " +
        ...
        try {
            ...
            if (!info)
                throw new Error(`Unknown market: ${market}. Use get_supported_markets to see available markets.`);
    ...
    server.registerTool("get_protocol_summary", {
    ...
    // ============================================================================
    server.registerTool("get_markets_from_api", {
    ...
    server.registerTool("get_dashboard_from_api", {
    ...
    server.registerTool("get_jtoken_details_from_api", {
    ...
    server.registerTool("get_account_summary", {
        description: "Get a comprehensive view of a user's JustLend positions: supply balances, borrow balances, " +
    ...
    server.registerTool("check_allowance", {
        description: "Check if the underlying TRC20 token has been approved for a jToken market. " +
    ...
    server.registerTool("get_trx_balance", {
    ...
    server.registerTool("get_account_data_from_api", {
     ...
     */
    async function getResourceWarning(ownerAddress, operation, isTRX, network, jTokenSymbol, amount) {
        try {
            const estimated = await services.simulateOperationResources(operation, jTokenSymbol || "", amount || "0", ownerAddress, network);
    ...
    server.registerTool("approve_underlying", {
    ...
    // ENERGY ESTIMATION
    // ============================================================================
    server.registerTool("estimate_lending_energy", {
        description: "Estimate energy, bandwidth, and TRX cost for any JustLend operation BEFORE executing it. " +
    ...
    // ============================================================================
    server.registerTool("get_proposal_list", {
    ...
    server.registerTool("get_user_vote_status", {
    ...
    server.registerTool("get_vote_info", {
        description: "Get voting power info for a user: JST wallet balance, available (surplus) votes, total deposited votes, and votes currently cast in proposals. " +
    ...
    server.registerTool("get_locked_votes", {
    ...
    server.registerTool("check_jst_allowance_for_voting", {
    ...
    server.registerTool("approve_jst_for_voting", {
        description: "Approve JST token for the WJST voting contract. Required before depositing JST to get voting power. " +
            "Use amount='max' for unlimited approval.",
        inputSchema: {
            amount: z.string().optional().describe("Amount to approve, or 'max' for unlimited. Default: max"),
            network: z.string().optional().describe("Network. Default: mainnet"),
    ...
    server.registerTool("deposit_jst_for_votes", {
        description: "Deposit JST into the WJST contract to get voting power. " +
    ...
    server.registerTool("withdraw_votes_to_jst", {
        description: "Withdraw WJST back to JST. Can only withdraw votes that are not currently locked in active proposals. " +
    ...
    server.registerTool("cast_vote", {
        description: "Cast a vote on a governance proposal. You must have available votes (deposit JST first if needed). " +
    ...
    server.registerTool("withdraw_votes_from_proposal", {
        description: "Withdraw (reclaim) votes from a completed or canceled proposal. " +
